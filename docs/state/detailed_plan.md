...
- P#63 [DONE]: Dedup/perf cleanup wave: unified pull-worker runtime/build path across built-ins/external/netflow, collapsed collector failover send duplication, precompiled netflow iface wildcard masks, reused Prometheus parser in script/http_server parse paths, lazy-created window maps after var filters, and removed test-only runtime wrappers with full `go test ./...` pass.
- P#64 [DONE]: LOC-reduction pass over P#63: simplified pull-worker helper surface in `engine` (removed extra behavior/build wrappers), kept dedup logic, and validated with `go test ./...`; net Go delta moved to negative.
- P#65 [DONE]: Ran 10-minute full integrated runtime test (all metric types) with pre-clean of current logs/tables, validated ClickHouse inserts/invariants (`dt<dts`, netflow last-only, external metric keys), and captured/analyzed pprof CPU+heap outputs.
