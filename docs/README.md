...
### NET
Системная метрика сетевых интерфейсов по байтам, пакетам и ошибкам.
- key: строковый идентификатор сущности метрики; имя сетевого интерфейса (`eth0`, `ens...`).
...
### NETFLOW
Системная pull-метрика top-N сетевых flow-пар по интерфейсам (без cgo), секция `[[metrics.netflow]]`.
