...
```
magent собирает системные метрики: утилизация сетевых интерфейсов, CPU, RAM, disk, fs, netflow-пары, сетевой стек, ядро, процессы и тд.  собирает  произвольные внешние метрики: выполняет скрипты, принимает через http_server или опрашивает через http_client.
...
- эффетивная фильтрация событий на стороне агента
- отказоусточивость. локальные очереди доставки и failover до коллектора
...
- агрегаты (процентили) пересчитываются один раз в период отправки по значениям собранных за периоды опроса. процентили опциональны и  назначаются в конфиге метрик
- по каждой метрике собирается последнее значение за период опроса и агрегаты за период отправки. 
...
- hot reload конфига выполняется по `SIGHUP`: агент перечитывает и валидирует `config.toml`, затем полностью пересоздаёт runtime (все воркеры/коллекторы/listeners); при ошибке применения автоматически возвращает предыдущий рабочий runtime
...
- метрики собираются по периоду опроса (scrape), агрегируются и отправляются в батч по периоду отправки (send). если процентили не указаны период опроса метрики (scrape) равен периоду отправи метрики(send)
...
- процентили (если включены) агрегируются один раз за период (send) по измеренным значениям по периодами scrape.
...
- для key=`total`:
  ...
  - udp_in_errors: UDP ошибки приема за интервал опроса. uint64, количество
