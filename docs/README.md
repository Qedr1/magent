...
## Принцип
- агент собирает системные метрики: утилизация сетевых интерфейсов, CPU, RAM, disk, fs, netflow-пары, сетевой стек, ядро, процессы и тд.  выполняет произвольные внешние скрипты. результат отправляет в коллектор (vector)
...
### NETFLOW
- встроенная pull-метрика сетевых flow-пар (без cgo), секция `[[metrics.netflow]]`
...
- key: `iface|proto|src_ip|src_port|dst_ip|dst_port`
